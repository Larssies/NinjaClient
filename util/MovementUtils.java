...
public class MovementUtils {
    ...
    public static boolean isOnGround(double height) {
        if (!mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer, mc.thePlayer.getBoundingBox().offset(0.0D, -height, 0.0D)).isEmpty()) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isOnGround(double height, EntityPlayer player) {
        if (!mc.theWorld.getCollidingBoundingBoxes(player, player.getBoundingBox().offset(0.0D, -height, 0.0D)).isEmpty()) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean canStep(double height) {
		...
		if ((!mc.theWorld
				.getCollidingBoundingBoxes(mc.thePlayer,
						mc.thePlayer.getBoundingBox().expand(0.1, 0, 0).offset(0.0D, height - 0.1, 0.0D))
				.isEmpty()
				&& mc.theWorld
						.getCollidingBoundingBoxes(mc.thePlayer,
								mc.thePlayer.getBoundingBox().expand(0.1, 0, 0).offset(0.0D, height + 0.1, 0.0D))
				...
				|| (!mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer,
						...
						.isEmpty()
						&& mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer,
								mc.thePlayer.getBoundingBox().expand(-0.1, 0, 0).offset(0.0D, height + 0.1,
										0.0D))
								.isEmpty())
				|| (!mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer,
						mc.thePlayer.getBoundingBox().expand(0, 0, 0.1).offset(0.0D, height - 0.1, 0.0D))
						.isEmpty()
						&& mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer,
								mc.thePlayer.getBoundingBox().expand(0, 0, 0.1).offset(0.0D, height + 0.1, 0.0D))
								.isEmpty())
				|| (!mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer,
						mc.thePlayer.getBoundingBox().expand(0, 0, -0.1).offset(0.0D, height - 0.1, 0.0D))
						.isEmpty()
						&& mc.theWorld.getCollidingBoundingBoxes(mc.thePlayer, mc.thePlayer.getBoundingBox()
